...
function PropertyEditor({
    ...
    property: Property;
    required: boolean;
    onChange: (prop: Property, newKey?: string) => void;
    onRemove: () => void;
    ...
    return (
        <div className="p-3 mb-4 bg-white border rounded">
            <div className="flex items-center gap-2 mb-2">
                ...
                <Input value={name} onChange={(e) => onChange(property, e.target.value)} className="w-32" placeholder="Field name" />
...
export default function ValidationSchemaEditor({ value, onChange }: { value: Schema; onChange: (schema: Schema) => void }) {
    ...
    function updateProperty(oldKey: string, prop: Property, newKey?: string) {
        if (newKey !== undefined && newKey !== oldKey) {
            const { [oldKey]: _, ...rest } = schema.properties;
            const updated = {
                ...schema,
                properties: {
                    ...rest,
                ...
                required: schema.required.includes(oldKey) 
                    ? [...schema.required.filter(r => r !== oldKey), newKey]
                    : schema.required
            };
            setSchema(updated);
            onChange(updated);
            ...
            <div>
                {Object.entries(schema.properties).map(([key, prop]) => (
                    <PropertyEditor
                        ...
                        onChange={(updatedProp, newKey) => updateProperty(key, updatedProp, newKey)}
