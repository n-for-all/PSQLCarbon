    ...
    return (
        <>
            <Modal
                ...
                modalHeading={"Create a new table"}
                modalLabel="Table"
                primaryButtonText={"Create"}
                secondaryButtonText={"Cancel"}>
                <Form aria-label="Table form" autoComplete="new-password">
                    <Input
                        id="collectionName"
                        ...
                        helperText="Enter the name of the table you want to create"
...
export const CollectionDeleteModal = ({ open, onClose, onSuccess, dbName, collectionName }: CollectionDeleteModalProps) => {
    ...
    const submitDelete = async () => {
        if (confirmName != collectionName) {
            setErrors({ collectionName: "The name does not match the table name" });
            return;
        }
        setStatus("active");
        fetcher.submit(
    ...
    return (
        <Modal
            ...
            <h3>
                Are you sure you want to drop the table <b>"{collectionName}"</b>?
            </h3>
            <div className="mb-4 text-xs">Dropping a table will delete all records in the table and this action cannot be undone</div>
            <Input
                id="name"
                type="text"
                labelText="Type the name of the table to confirm"
                autoComplete="new-password"
                required
